...
function _joinRoom(room, currentRooms, socket, io){
  socket.join(room);
	currentRooms[socket.id] = room;
	io.sockets.emit('roomUpdate', {currentRooms: currentRooms});
};

function _handleRoomChangeRequests(data, currentRooms) {
	if (data.roomName === currentRooms[socket.id]) {
		...
		socket.emit('roomChangeResult', {
			success: true,
			message: "you are now in room: " + data.roomName
		});
	}
};
...
function createChat(server) {
	...
	io.on('connection', function (socket) {
		...
		_joinRoom("lobby", currentRooms, socket, io);
		...
		socket.on('nicknameChangeRequest', function(data){
			if (data.text.indexOf("user") === 0) {
				...
				io.to(currentRooms[socket.id]).emit('nicknameChangeUpdate', {
					users: nicknames
					});
				socket.emit('nicknameChangeResult', {
					success: true,
