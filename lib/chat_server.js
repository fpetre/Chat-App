...
function _joinRoom(room, currentRooms, socket){
  socket.join(room);
...
function createChat(server) {
	...
	io.on('connection', function (socket) {
		...
		_joinRoom("lobby", currentRooms, socket);
		...
		socket.on('disconnect', function(data){
			io.to(currentRooms[socket.id]).emit('userDisconnect', {user: nicknames[socket.id]});
		...
		socket.on('nicknameChangeRequest', function(data){
			if (data.text.indexOf("user") === 0) {
				...
				io.to(currentRooms[socket.id]).emit('nicknameChangeUpdate', {users: nicknames});
				socket.emit('nicknameChangeResult', {
					success: true,
